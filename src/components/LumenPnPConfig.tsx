// =============================================================================
// HYDRA-UMC STUDIO - React Component: LumenPnPConfig.tsx
...
/**
 * Executes the  lumen pn p config logic. 
 * This function handles the necessary computations and state updates.
 */
export function LumenPnPConfig() {
  ...
  const moduleData = selectedRobot.lumenPnP as any;
  const isEnabled = moduleData?.enabled || false;

  const handleToggle = () => {
    updateRobot(selectedRobot.id, {
      lumenPnP: { ...moduleData, enabled: !isEnabled }
    } as any);
  };

  const handleSizeChange = (axis: 'width' | 'length', value: number) => {
    updateRobot(selectedRobot.id, {
      lumenPnP: { ...moduleData, size: { ...moduleData.size, [axis]: value } }
    } as any);
  };
  ...
  const handleReset = () => {
    updateRobot(selectedRobot.id, {
      lumenPnP: { enabled: true, size: { width: 500, length: 500 }, worldPos: { x: 0, y: 0 }, worldRot: 0, renderScale: 1 }
    } as any);
  };
    ...
    <div className="h-full flex flex-col space-y-4">
      <div className="flex items-center justify-between shrink-0">
        <h2 className="text-lg font-semibold text-slate-100 flex items-center gap-2">
          <Cpu className="text-sky-400" size={20} /> LumenPnP
        </h2>
      ...
      {!isEnabled ? (
        <div className="bg-slate-900 border border-slate-800 rounded-lg p-8 flex flex-col items-center justify-center text-center flex-1">
          ...
          <h3 className="text-slate-200 font-medium mb-2">No LumenPnP Assigned</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 flex-1 min-h-0">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-lg overflow-hidden relative h-[400px] md:h-auto min-h-[400px]">
            ...
            <Canvas camera={{ position: [0.6, 0.6, 0.6], fov: 50 }} shadows className="w-full h-full outline-none">
              ...
              {moduleData && <SharedModule3DView module={moduleData} type="lumenPnP" />}
