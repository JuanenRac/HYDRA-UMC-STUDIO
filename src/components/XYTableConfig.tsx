...
import { Crosshair, ArrowUp, ArrowDown, ArrowLeft, ArrowRight, Save, Maximize2 } from 'lucide-react';
...
export function XYTableConfig() {
  const { robots, xyTable, updateXYTable } = useHydraStore();
  ...
  const handleJog = (axis: 'x' | 'y', direction: number) => {
    ...
    updateXYTable({
      pos: { ...xyTable.pos, [axis]: newPos }
    });
  };

  const handleSizeChange = (axis: 'width' | 'length', value: number) => {
    updateXYTable({
      tableSize: { ...xyTable.tableSize, [axis]: value }
    });
  };
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="space-y-4">
          <div className="bg-slate-900 border border-slate-800 rounded-lg p-4">
            <h3 className="text-sm font-medium text-slate-200 mb-2">Table Settings</h3>
            
            <div className="space-y-4">
              <div>
                <label className="block text-[11px] font-medium text-slate-400 mb-1">Target Robot</label>
                <select 
                  ...
                  value={xyTable.assignedRobotId || ''}
                  onChange={(e) => updateXYTable({ assignedRobotId: e.target.value ? Number(e.target.value) : null })}
                >
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input 
                    ...
                    min="100" max="5000" step="10"
                    className="w-full bg-slate-950 border border-slate-700 rounded px-3 py-2 min-h-[40px] text-xs text-slate-200 focus:outline-none focus:border-amber-500"
                    value={xyTable.tableSize.length}
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-lg p-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-slate-950 border border-slate-800 rounded p-3 flex flex-col items-center gap-2">
                ...
                <div className="text-lg font-mono text-amber-400">{xyTable.pos.x.toFixed(2)}</div>
              ...
              <div className="bg-slate-950 border border-slate-800 rounded p-3 flex flex-col items-center gap-2">
                ...
                <div className="text-lg font-mono text-amber-400">{xyTable.pos.y.toFixed(2)}</div>
                <div className="flex gap-2 mt-1 w-full justify-center">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-lg overflow-hidden relative h-[400px] md:h-auto min-h-[400px]">
          ...
          <Canvas camera={{ position: [0.6, 0.6, 0.6], fov: 50 }} shadows className="w-full h-full outline-none">
            ...
            <XYTableVisualizer xyTable={xyTable} />
