// =============================================================================
// HYDRA-UMC STUDIO - Unitree Z1-specific kinematics: z1Kinematics.ts
// Copyright (C) 2026 JuanenRac (Electro Hobby 3D) <electrohobby3d@gmail.com>
// GPL-3.0 - see LICENSE
//
// Chain offsets copied verbatim from mujoco_menagerie's own
// unitree_z1/z1.xml MJCF (BSD-3-Clause - see public/models/z1/
// ATTRIBUTION.txt). Every joint's own rpy is (0,0,0) (pure translation)
// but the axis varies per joint (not always Z) - joint2/3/4 rotate about
// Y, joint5 about Z, joint6 about X - so this uses the same
// "quaternion family" approach as M-710iC (see that file's own header).
// =============================================================================

import { Matrix4, Quaternion, Vector3, Euler } from 'three';
import type { KinematicsPoint } from './utils';
import { Z1_CHAIN, Z1_ROOT_QUAT } from '../components/3d/Z1Arm';
...
const ROOT_M = new Matrix4().makeRotationFromQuaternion(Z1_ROOT_QUAT);
...
export function z1JointsToCartesian(pt: KinematicsPoint): { x: number; y: number; z: number; a: number; b: number; c: number } {
...
export function z1CartesianToJoints(x: number, y: number, z: number, a: number, b: number, c: number): KinematicsPoint {
  const { j1, j2, j3 } = solveJ1J2J3(x / 1000, (z + Z_OFFSET_MM) / 1000, y / 1000);
  return { j1, j2, j3, j4: a || 0, j5: b || 0, j6: c || 0 };
}

// From the source MJCF's own <keyframe> "home" qpos.
export const Z1_HOME_POSE: KinematicsPoint = { j1: 0, j2: 44.98, j3: -14.96, j4: -29.97, j5: 0, j6: 0 };
