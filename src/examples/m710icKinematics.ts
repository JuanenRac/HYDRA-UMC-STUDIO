// =============================================================================
// HYDRA-UMC STUDIO - FANUC M-710iC-specific kinematics: m710icKinematics.ts
// Copyright (C) 2026 JuanenRac (Electro Hobby 3D) <electrohobby3d@gmail.com>
// GPL-3.0 - see LICENSE
//
// Chain offsets copied verbatim from the official
// robot-descriptions/fanuc_m710ic_description repo's own
// urdf/m710ic70.urdf (BSD-3-Clause - see public/models/m710ic/
// ATTRIBUTION.txt). Every joint's own <origin rpy> is (0,0,0) here (pure
// translation between joints), but each joint's own <axis> is a
// different cardinal direction (not always Z like the UR family) - e.g.
// joint_2 rotates about world Y, joint_4/joint_6 about -X - so this uses
// the same "quaternion family" approach as Parol6/Faze4/AR3/AR4/e.DO
// (see Faze4Arm.tsx's own header for why that engine exists), even
// though every rpy here happens to be zero.
// =============================================================================

import { Matrix4, Quaternion, Vector3, Euler } from 'three';
import type { KinematicsPoint } from './utils';
import { M710IC_CHAIN, M710IC_ROOT_QUAT } from '../components/3d/M710icArm';
...
const ROOT_M = new Matrix4().makeRotationFromQuaternion(M710IC_ROOT_QUAT);

function fkPosition(j1: number, j2: number, j3: number): Vector3 {
  let m = ROOT_M.clone();
  const joints = [j1, j2, j3, 0, 0, 0];
  for (let i = 0; i < 6; i++) {
    const c = M710IC_CHAIN[i];
...
export function m710icJointsToCartesian(pt: KinematicsPoint): { x: number; y: number; z: number; a: number; b: number; c: number } {
...
export function m710icCartesianToJoints(x: number, y: number, z: number, a: number, b: number, c: number): KinematicsPoint {
  const { j1, j2, j3 } = solveJ1J2J3(x / 1000, (z + Z_OFFSET_MM) / 1000, y / 1000);
  return { j1, j2, j3, j4: a || 0, j5: b || 0, j6: c || 0 };
}

export const M710IC_HOME_POSE: KinematicsPoint = { j1: 0, j2: 0, j3: 0, j4: 0, j5: 0, j6: 0 };
