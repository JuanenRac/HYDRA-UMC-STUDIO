// =============================================================================
// HYDRA-UMC STUDIO - e.DO-specific kinematics: edoKinematics.ts
// Copyright (C) 2026 JuanenRac (Electro Hobby 3D) <electrohobby3d@gmail.com>
// GPL-3.0 - see LICENSE
//
// Chain offsets copied verbatim from Comau's own official
// robots/edo_sim.urdf (github.com/ianathompson/eDO_description,
// BSD-3-Clause - see public/models/edo/ATTRIBUTION.txt). Unlike xArm6/
// Lite6 (which share the UR family's "every joint is local Z" structure),
// e.DO's own <axis> per joint is genuinely arbitrary - joint_2's own axis
// is (-0.88847, 0.2908, 0.35504), not a cardinal direction - so this uses
// the same "quaternion family" approach as Parol6/Faze4/AR3/AR4 (see
// Faze4Arm.tsx's own header for why that engine exists), with a real
// 3-parameter (j1,j2,j3) Newton-Raphson position solve like Faze4's own
// (not Parol6's simpler 2-parameter one), since e.DO's j1 does not cleanly
// separate into "radius/height around a vertical axis" either.
// =============================================================================

import { Matrix4, Quaternion, Vector3, Euler } from 'three';
import type { KinematicsPoint } from './utils';
import { EDO_CHAIN, EDO_ROOT_QUAT } from '../components/3d/EdoArm';
...
const ROOT_M = new Matrix4().makeRotationFromQuaternion(EDO_ROOT_QUAT);
...
// app z=0 maps to this many mm above e.DO's own neutral (j2=j3=0) pose
// height - same reasoning as faze4Kinematics.ts's own Z_OFFSET_MM,
// computed from the actual neutral-pose FK rather than hand-tuned.
const Z_OFFSET_MM = (() => {
  const p = fkPosition(0, 0, 0);
  return p.y * 1000;
})();

export function edoJointsToCartesian(pt: KinematicsPoint): { x: number; y: number; z: number; a: number; b: number; c: number } {
...
export function edoCartesianToJoints(x: number, y: number, z: number, a: number, b: number, c: number): KinematicsPoint {
  const { j1, j2, j3 } = solveJ1J2J3(x / 1000, (z + Z_OFFSET_MM) / 1000, y / 1000);
  return { j1, j2, j3, j4: a || 0, j5: b || 0, j6: c || 0 };
}

// e.DO's own "home" position (Comau's default sim pose is all-zero, a
// real non-self-colliding straight-up pose for this robot's geometry).
export const EDO_HOME_POSE: KinematicsPoint = { j1: 0, j2: 0, j3: 0, j4: 0, j5: 0, j6: 0 };
