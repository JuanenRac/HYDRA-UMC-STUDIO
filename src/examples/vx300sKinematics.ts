// =============================================================================
// HYDRA-UMC STUDIO - ViperX 300-specific kinematics: vx300sKinematics.ts
// Copyright (C) 2026 JuanenRac (Electro Hobby 3D) <electrohobby3d@gmail.com>
// GPL-3.0 - see LICENSE
//
// Chain offsets copied verbatim from Interbotix's own official
// urdf/vx300s.urdf.xacro (BSD-3-Clause - see public/models/vx300s/
// ATTRIBUTION.txt). Every joint's own rpy is (0,0,0) (pure translation)
// but the axis varies per joint - same "quaternion family" position
// solve as M-710iC/Z1 (mesh visual offsets don't affect FK position at
// all, only where each link's own mesh is drawn - see Vx300sArm.tsx).
// =============================================================================

import { Matrix4, Quaternion, Vector3, Euler } from 'three';
import type { KinematicsPoint } from './utils';
import { VX300S_CHAIN, VX300S_ROOT_QUAT } from '../components/3d/Vx300sArm';
...
const ROOT_M = new Matrix4().makeRotationFromQuaternion(VX300S_ROOT_QUAT);

function fkPosition(j1: number, j2: number, j3: number): Vector3 {
  let m = ROOT_M.clone();
  const joints = [j1, j2, j3, 0, 0, 0];
  for (let i = 0; i < 6; i++) {
    const c = VX300S_CHAIN[i];
...
export function vx300sJointsToCartesian(pt: KinematicsPoint): { x: number; y: number; z: number; a: number; b: number; c: number } {
...
export function vx300sCartesianToJoints(x: number, y: number, z: number, a: number, b: number, c: number): KinematicsPoint {
  const { j1, j2, j3 } = solveJ1J2J3(x / 1000, (z + Z_OFFSET_MM) / 1000, y / 1000);
  return { j1, j2, j3, j4: a || 0, j5: b || 0, j6: c || 0 };
}

export const VX300S_HOME_POSE: KinematicsPoint = { j1: 0, j2: 0, j3: 0, j4: 0, j5: 0, j6: 0 };
