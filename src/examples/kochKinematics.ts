// =============================================================================
// HYDRA-UMC STUDIO - Koch v1.1-specific kinematics: kochKinematics.ts
// Copyright (C) 2026 JuanenRac (Electro Hobby 3D) <electrohobby3d@gmail.com>
// GPL-3.0 - see LICENSE
//
// Chain offsets copied verbatim from mujoco_menagerie's own
// low_cost_robot_arm/low_cost_robot_arm.xml MJCF (Apache-2.0 - see
// public/models/koch/ATTRIBUTION.txt). Covers BOTH "Koch v1.1" and
// "Low-Cost Robot Arm" - the same real open-hardware design.
//
// Only 5 real arm joints (not 6 - the source MJCF's own 6th joint is
// the gripper jaw, not a wrist orientation axis), same situation as
// SO-ARM100 (see soArm100Kinematics.ts's own header for the general
// approach this mirrors).
// =============================================================================

import { Matrix4, Quaternion, Vector3, Euler } from 'three';
import type { KinematicsPoint } from './utils';
import { KOCH_CHAIN, KOCH_ROOT_QUAT } from '../components/3d/KochArm';
...
const ROOT_M = new Matrix4().makeRotationFromQuaternion(KOCH_ROOT_QUAT);

function fkPosition(j1: number, j2: number, j3: number): Vector3 {
  let m = ROOT_M.clone();
  // Only 5 real joints - j4 (wrist_pitch)/j5 (wrist_roll) held at 0 for
  // the position-only solve, same reasoning as soArm100Kinematics.ts.
  const joints = [j1, j2, j3, 0, 0];
  for (let i = 0; i < 5; i++) {
    const c = KOCH_CHAIN[i];
...
export function kochJointsToCartesian(pt: KinematicsPoint): { x: number; y: number; z: number; a: number; b: number; c: number } {
...
export function kochCartesianToJoints(x: number, y: number, z: number, a: number, b: number, c: number): KinematicsPoint {
  const { j1, j2, j3 } = solveJ1J2J3(x / 1000, (z + Z_OFFSET_MM) / 1000, y / 1000);
  return { j1, j2, j3, j4: a || 0, j5: b || 0, j6: c || 0 };
}

export const KOCH_HOME_POSE: KinematicsPoint = { j1: 0, j2: 0, j3: 0, j4: 0, j5: 0, j6: 0 };
