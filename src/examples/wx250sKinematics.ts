// =============================================================================
// HYDRA-UMC STUDIO - WidowX 250-specific kinematics: wx250sKinematics.ts
// Copyright (C) 2026 JuanenRac (Electro Hobby 3D) <electrohobby3d@gmail.com>
// GPL-3.0 - see LICENSE
//
// Same structure as vx300sKinematics.ts (see that file's own header) -
// WidowX 250 is Trossen's smaller sibling to ViperX 300, sharing the
// exact same rig design, just different link lengths. Chain copied
// verbatim from Interbotix's own urdf/wx250s.urdf.xacro (BSD-3-Clause -
// see public/models/wx250s/ATTRIBUTION.txt).
// =============================================================================

import { Matrix4, Quaternion, Vector3, Euler } from 'three';
import type { KinematicsPoint } from './utils';
import { WX250S_CHAIN, WX250S_ROOT_QUAT } from '../components/3d/Wx250sArm';
...
const ROOT_M = new Matrix4().makeRotationFromQuaternion(WX250S_ROOT_QUAT);

function fkPosition(j1: number, j2: number, j3: number): Vector3 {
  let m = ROOT_M.clone();
  const joints = [j1, j2, j3, 0, 0, 0];
  for (let i = 0; i < 6; i++) {
    const c = WX250S_CHAIN[i];
...
export function wx250sJointsToCartesian(pt: KinematicsPoint): { x: number; y: number; z: number; a: number; b: number; c: number } {
...
export function wx250sCartesianToJoints(x: number, y: number, z: number, a: number, b: number, c: number): KinematicsPoint {
  const { j1, j2, j3 } = solveJ1J2J3(x / 1000, (z + Z_OFFSET_MM) / 1000, y / 1000);
  return { j1, j2, j3, j4: a || 0, j5: b || 0, j6: c || 0 };
}

export const WX250S_HOME_POSE: KinematicsPoint = { j1: 0, j2: 0, j3: 0, j4: 0, j5: 0, j6: 0 };
