...
## Features
- 🤖 **Multi-Robot Control**: Manage multiple 6-DOF robots simultaneously, grouped by manufacturer in the model picker (Config tab). **Source Robotics** (Parol6, Faze4), **Annin Robotics** (AR3, AR4), and **Universal Robots** (UR3e, UR5e, UR10e, UR16e, UR20 - official geometry/kinematics from `github.com/UniversalRobots/Universal_Robots_ROS2_Description`, BSD-3-Clause) are all modeled with their real mesh geometry and their manufacturer's own joint kinematics (accurate forward/inverse kinematics, real joint limits where the robot defines any); a Generic simplified two-link arm is also available for any other rig. License/source for every model's own mesh files is documented in that model's own `public/models/<slug>/ATTRIBUTION.txt`.
